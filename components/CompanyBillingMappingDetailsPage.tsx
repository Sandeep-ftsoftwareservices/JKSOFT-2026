...
export const CompanyBillingMappingDetailsPage: React.FC<MappingDetailsPageProps> = ({ 
  mapping, companies, billingTypes, onNavigate, onEdit 
}) => {
  const company = companies.find(c => c.id === mapping.companyId);
  const assignedTypes = billingTypes.filter(t => mapping.billingTypeIds.includes(t.id));
  ...
  return (
    <div className="px-4 md:p-8 lg:px-12 flex flex-col flex-1 overflow-y-auto">
      <div className="max-w-5xl mx-auto flex flex-col flex-1 w-full gap-6">
        <Breadcrumbs currentView="view-billing-mapping" onNavigate={onNavigate} />
        
        <div className="flex flex-wrap items-center justify-between gap-4 px-4 pb-2">
          <div className="flex flex-col gap-1">
            <h1 className="text-[#111418] dark:text-white text-3xl font-black tracking-tight">{company?.fullName} Billing Configuration</h1>
            <p className="text-[#617589] dark:text-gray-400 text-sm">Review enabled billing methods and assignment history.</p>
          </div>
          <div className="flex items-center gap-3">
             <button 
              onClick={() => onNavigate('billing-mapping')}
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-white dark:bg-[#1a2632] rounded-xl shadow-sm border border-[#e5e7eb] dark:border-gray-800 overflow-hidden">
              <div className="px-6 py-4 border-b border-[#f0f2f4] dark:border-[#2e3b4b] flex items-center gap-2">
                ...
                <h3 className="font-bold text-[#111418] dark:text-white">Active Assignments</h3>
              </div>
              <div className="p-6">
                 {assignedTypes.length > 0 ? (
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                     {assignedTypes.map(t => (
                       <div key={t.id} className="p-4 rounded-lg border border-[#f0f2f4] dark:border-gray-800 bg-gray-50 dark:bg-slate-800/50">
                         <h4 className="font-bold text-sm dark:text-white">{t.name}</h4>
                         <p className="text-xs text-[#617589] mt-1">{t.description}</p>
                       </div>
                     ))}
                   </div>
                 ) : (
                   <p className="text-gray-500 italic text-sm text-center py-8">No billing types assigned to this company.</p>
                 )}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#1a2632] rounded-xl shadow-sm border border-[#e5e7eb] dark:border-gray-800 p-6 flex items-center gap-4">
              ...
              <div>
                <h4 className="font-bold dark:text-white">Status: {mapping.isDeleted ? 'Deleted' : mapping.isActive ? 'Active' : 'Inactive'}</h4>
                <p className="text-xs text-gray-500">This configuration is {mapping.isActive && !mapping.isDeleted ? 'currently operational' : 'temporarily suspended'}.</p>
              </div>
            </div>
          </div>
