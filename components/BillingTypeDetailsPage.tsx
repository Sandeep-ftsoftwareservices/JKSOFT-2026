...
interface BillingTypeDetailsPageProps {
  billingType: BillingType;
  onNavigate: (view: ViewType) => void;
  onEdit: (billingType: BillingType) => void;
}

export const BillingTypeDetailsPage: React.FC<BillingTypeDetailsPageProps> = ({ 
  billingType, onNavigate, onEdit 
}) => {
  return (
    <div className="px-4 md:p-8 lg:px-12 flex flex-col flex-1 overflow-y-auto">
      <div className="max-w-5xl mx-auto flex flex-col flex-1 w-full gap-6">
        <Breadcrumbs currentView="view-billing-type" onNavigate={onNavigate} />
        
        <div className="flex flex-wrap items-center justify-between gap-4 px-4 pb-2">
          <div className="flex flex-col gap-1">
            <h1 className="text-[#111418] dark:text-white text-3xl font-black tracking-tight">{billingType.name}</h1>
            <p className="text-[#617589] dark:text-gray-400 text-sm">Detailed overview of this billing type configuration.</p>
          </div>
          <div className="flex items-center gap-3">
             <button 
              onClick={() => onNavigate('billing-types')}
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-white dark:bg-[#1a2632] rounded-xl shadow-sm border border-[#e5e7eb] dark:border-gray-800 overflow-hidden">
              <div className="px-6 py-4 border-b border-[#f0f2f4] dark:border-[#2e3b4b] flex items-center gap-2">
                ...
                <h3 className="font-bold text-[#111418] dark:text-white">Configuration</h3>
              </div>
              <div className="p-6 space-y-6">
                <div className="flex flex-col gap-1">
                  <span className="text-[10px] font-bold text-[#617589] uppercase tracking-wider">Billing Type Name</span>
                  <span className="text-sm font-semibold text-[#111418] dark:text-gray-200">{billingType.name}</span>
                </div>
                <div className="flex flex-col gap-1">
            ...
            <div className="bg-white dark:bg-[#1a2632] rounded-xl shadow-sm border border-[#e5e7eb] dark:border-gray-800 p-6 flex items-center gap-4">
              <div className={`size-12 rounded-full flex items-center justify-center ${billingType.isActive ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                <span className="material-symbols-outlined text-[24px]">{billingType.isActive ? 'check_circle' : 'cancel'}</span>
              </div>
              <div>
                <h4 className="font-bold dark:text-white">Status: {billingType.isActive ? 'Active' : 'Inactive'}</h4>
                <p className="text-xs text-gray-500">This billing type is {billingType.isActive ? 'currently available' : 'disabled'} for business entity mapping.</p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white dark:bg-[#1a2632] rounded-xl shadow-sm border border-[#e5e7eb] dark:border-gray-800 overflow-hidden">
               <div className="px-6 py-4 border-b border-[#f0f2f4] dark:border-[#2e3b4b] flex items-center gap-2">
                ...
                <div className="p-6 space-y-5">
                   {[
                    { label: 'Created By', value: billingType.audit.createdBy, icon: 'person_add' },
                    { label: 'Created Date', value: billingType.audit.createdDate, icon: 'calendar_today' },
                    { label: 'Updated By', value: billingType.audit.updatedBy, icon: 'edit_note' },
                    { label: 'Updated Date', value: billingType.audit.updatedDate, icon: 'update' }
                  ].map((item, i) => (
