...
interface ColonyListPageProps {
  colonies: ColonyDetails[];
  onEdit: (colony: ColonyDetails) => void;
  onView: (colony: ColonyDetails) => void;
  onAdd: () => void;
...
type SortConfig = {
  key: keyof ColonyDetails;
  direction: 'asc' | 'desc';
} | null;

export const ColonyListPage: React.FC<ColonyListPageProps> = ({ 
  colonies, 
  onEdit, 
  onView,
  onAdd, 
  ...
  const handleSort = (key: keyof ColonyDetails) => {
  ...
  const processedColonies = useMemo(() => {
    ...
    return result;
  }, [colonies, searchTerm, sortConfig]);

  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm]);

  const totalPages = Math.ceil(processedColonies.length / pageSize);
  const paginatedColonies = useMemo(() => {
    const start = (currentPage - 1) * pageSize;
    return processedColonies.slice(start, start + pageSize);
  }, [processedColonies, currentPage]);

  const HeaderCell = ({ label, columnKey }: { label: string, columnKey: keyof ColonyDetails }) => (
    <th 
      ...
      <div className="max-w-5xl mx-auto flex flex-col flex-1 w-full gap-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 px-4">
          <div className="flex min-w-72 flex-col gap-2">
            <h1 className="text-[#111418] dark:text-white tracking-light text-[32px] font-bold leading-tight">Colonies</h1>
            <p className="text-[#617589] dark:text-gray-400 text-sm font-normal leading-normal">Manage local colonies and neighborhood clusters.</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 dark:bg-[#25323e] border-b border-[#dbe0e6] dark:border-[#2e3b4b]">
                  ...
                  <HeaderCell label="Area" columnKey="area" />
              ...
              <tbody className="divide-y divide-[#dbe0e6] dark:divide-[#2e3b4b]">
                {paginatedColonies.map((colony) => (
                  <tr key={colony.id} className="hover:bg-gray-50 dark:hover:bg-[#25323e]/50 transition-colors group">
                    <td className="p-4 text-sm font-medium text-[#111418] dark:text-white">{colony.fullName}</td>
                    <td className="p-4 text-sm text-[#617589] dark:text-gray-300">{colony.shortName}</td>
                    <td className="p-4 text-sm text-[#617589] dark:text-gray-300">{colony.area}</td>
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${colony.isActive ? 'bg-green-500' : 'bg-gray-400'}`}></span>
                        {colony.isActive ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          onClick={() => onView(colony)}
              ...
              <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm text-gray-700 dark:text-gray-400">
                    Showing <span className="font-medium text-[#111418] dark:text-white">{(currentPage - 1) * pageSize + 1}</span> to <span className="font-medium text-[#111418] dark:text-white">{Math.min(currentPage * pageSize, processedColonies.length)}</span> of <span className="font-medium text-[#111418] dark:text-white">{processedColonies.length}</span> results
                  </p>
                </div>
                <div>
