...
export const StateDetailsPage: React.FC<StateDetailsPageProps> = ({ state, onNavigate, onEdit }) => {
  const infoGroups = [
    {
      title: 'General Information',
      icon: 'info',
      items: [
        { label: 'Full State Name', value: state.fullName },
        { label: 'Short Name', value: state.shortName },
        { label: 'State Code', value: state.stateCode, mono: true }
      ]
    },
    {
      title: 'Regional & Tax Config',
      icon: 'payments',
      items: [
        { label: 'Parent Country', value: state.country },
        { label: 'GST Code', value: state.gstCode, mono: true },
        { label: 'STD Code', value: state.stdCode, mono: true }
      ]
    }
  ];
  ...
  return (
    <div className="px-4 md:p-8 lg:px-12 flex flex-col flex-1 overflow-y-auto">
      <div className="max-w-5xl mx-auto flex flex-col flex-1 w-full gap-6">
        <Breadcrumbs currentView="view-state" onNavigate={onNavigate} />
        
        <div className="flex flex-wrap items-center justify-between gap-4 px-4 pb-2">
          <div className="flex flex-col gap-1">
            <h1 className="text-[#111418] dark:text-white text-3xl font-black tracking-tight">{state.fullName}</h1>
            <p className="text-[#617589] dark:text-gray-400 text-sm">Detailed configuration overview for this state record.</p>
          </div>
        ...
        <div className="mx-4 grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-[#1a2632] rounded-xl shadow-sm border border-[#e5e7eb] dark:border-gray-800 p-6 flex flex-col items-center text-center gap-4">
              ...
              <div className={`size-16 rounded-full flex items-center justify-center ${state.isActive ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                <span className="material-symbols-outlined text-[32px]">{state.isActive ? 'check_circle' : 'cancel'}</span>
              </div>
              <div>
                <h4 className="font-bold text-lg dark:text-white">{state.isActive ? 'Active' : 'Inactive'}</h4>
                <p className="text-xs text-gray-500 max-w-[160px]">This state is currently {state.isActive ? 'operational' : 'disabled'} in the ecosystem.</p>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#1a2632] rounded-xl shadow-sm border border-[#e5e7eb] dark:border-gray-800 overflow-hidden">
               <div className="px-6 py-4 border-b border-[#f0f2f4] dark:border-[#2e3b4b] flex items-center gap-2">
                ...
                <div className="p-6 space-y-4">
                   {[
                    { label: 'Created By', value: state.audit.createdBy },
                    { label: 'Created Date', value: state.audit.createdDate },
                    { label: 'Updated By', value: state.audit.updatedBy },
                    { label: 'Updated Date', value: state.audit.updatedDate }
                  ].map((item, i) => (
