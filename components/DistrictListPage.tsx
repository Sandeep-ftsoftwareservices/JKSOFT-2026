...
import { DistrictDetails, ViewType } from '../types';
import { Breadcrumbs } from './Breadcrumbs';

interface DistrictListPageProps {
  districts: DistrictDetails[];
  onEdit: (district: DistrictDetails) => void;
  onView: (district: DistrictDetails) => void;
  onAdd: () => void;
...
type SortConfig = {
  key: keyof DistrictDetails;
  direction: 'asc' | 'desc';
} | null;

export const DistrictListPage: React.FC<DistrictListPageProps> = ({ 
  districts, 
  onEdit, 
  onView,
  onAdd, 
  ...
  const handleSort = (key: keyof DistrictDetails) => {
  ...
  const processedDistricts = useMemo(() => {
    ...
    return result;
  }, [districts, searchTerm, sortConfig]);

  const HeaderCell = ({ label, columnKey }: { label: string, columnKey: keyof DistrictDetails }) => (
    <th 
  ...
  return (
    <div className="px-4 md:p-8 lg:px-12 flex flex-col flex-1 overflow-y-auto">
      <div className="max-w-5xl mx-auto flex flex-col flex-1 w-full gap-6">
        <Breadcrumbs currentView="districts" onNavigate={onNavigate} />
        
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 px-4">
          <div className="flex min-w-72 flex-col gap-2">
            <h1 className="text-[#111418] dark:text-white tracking-light text-[32px] font-bold leading-tight">Districts</h1>
            <p className="text-[#617589] dark:text-gray-400 text-sm font-normal leading-normal">Manage the list of districts available in the system.</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#dbe0e6] dark:divide-[#2e3b4b]">
                {processedDistricts.map((district) => (
                  <tr key={district.id} className="hover:bg-gray-50 dark:hover:bg-[#25323e]/50 transition-colors group">
                    <td className="p-4 text-sm font-medium text-[#111418] dark:text-white">{district.fullName}</td>
                    <td className="p-4 text-sm text-[#617589] dark:text-gray-300">{district.shortName}</td>
                    <td className="p-4 text-sm text-[#617589] dark:text-gray-300">{district.city}</td>
                    <td className="p-4 text-sm text-[#617589] dark:text-gray-300">{district.state}</td>
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${district.isActive ? 'bg-green-500' : 'bg-gray-400'}`}></span>
            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700 dark:text-gray-400">
                  Showing <span className="font-medium text-[#111418] dark:text-white">1</span> to <span className="font-medium text-[#111418] dark:text-white">{processedDistricts.length}</span> of <span className="font-medium text-[#111418] dark:text-white">{processedDistricts.length}</span> results
                </p>
              </div>
              <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm">
                <button className="relative inline-flex items-center rounded-l-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:ring-gray-600 dark:hover:bg-[#15202b]">
                ...
                <button className="relative inline-flex items-center rounded-r-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:ring-gray-600 dark:hover:bg-[#15202b]">
